...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-2">Réservations à venir</h3>
            <p class="text-3xl font-bold text-green-600">{{ $stats['upcoming_bookings'] }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-2">Réservations terminées</h3>
            <p class="text-3xl font-bold text-gray-600">{{ $stats['completed_bookings'] }}</p>
        </div>
    </div>
        ...
        <div class="space-y-4">
            ...
            <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="font-semibold">{{ $booking->trip->departure_city }} → {{ $booking->trip->arrival_city }}</h3>
                        <p class="text-sm text-gray-600">{{ $booking->trip->departure_date->format('d/m/Y') }} à {{ date('H:i', strtotime($booking->trip->departure_time)) }}</p>
                        <p class="text-sm text-gray-600 mt-1">Type: {{ $booking->isPassenger() ? 'Passager' : 'Colis' }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-lg font-bold text-blue-600">{{ number_format($booking->price, 2) }} €</p>
                ...
                <div class="mt-4">
                    <a href="{{ route('bookings.show', $booking) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">Voir les détails →</a>
                </div>
            </div>
            @endforeach
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm text-gray-500">{{ $booking->trip->departure_date->format('d/m/Y') }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
